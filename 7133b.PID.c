...
#pragma config(Motor,  port1,           L2,            tmotorVex393_HBridge, openLoop)
#pragma config(Motor,  port2,           Drive,         tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port4,           L1,            tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port6,           L3,            tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port7,           Drive2,        tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port8,           RIGHT,         tmotorVex393_MC29, openLoop, encoderPort, I2C_2)
#pragma config(Motor,  port9,           LEFT,          tmotorVex393_MC29, openLoop, encoderPort, I2C_1)
#pragma config(Motor,  port10,          L4,            tmotorVex393_HBridge, openLoop)
...
//int threshold = 505; //take reading on light, dark, add, divide by 2
...
leftTurns = SensorValue(rightEncoder);
rightTurns = SensorValue(leftEncoder);
...
motor[RIGHT] = 127;
motor[LEFT] = 127;
while((sensorValue(leftEncoder) < 381) && (sensorValue(rightEncoder) > -381))
{
leftTurns = SensorValue(rightEncoder);
rightTurns = SensorValue(leftEncoder);
}*/
...
motor[L4] = 0;
SensorValue(rightEncoder) = 0;
while (SensorValue(rightEncoder)<4500)
{
...
if ((SensorValue(line)<threshold)&&(SensorValue(lineLeft)>threshold)&&(SensorValue(lineRight)>threshold))
{
...
else if ((SensorValue(lineLeft)<threshold)&&(SensorValue(lineRight)>threshold))
...
motor[RIGHT] = -127;
motor[LEFT] = 127;
while((sensorValue(rightEncoder) < 127) && (sensorValue(leftEncoder) < -127))
{
